...
for (i in 1:NROW(wtgNotReachRated)) {
  ...
  #Powercureve Plot
  rated_power <- unique(csvPower$Rating)
  powercurve <- csvPower %>%
      ggplot(aes(x = WindSpeed, y = RealPower)) +
      geom_point(color = "#00689C", size = .5) +
      geom_hline(yintercept = rated_power,
                 color = "#CA0123",
                 linewidth = .5,  # Made thicker so you can definitely see it
                 linetype = "dashed") +
      labs(x = "Windspeed", 
           y = "RealPower", 
           title = paste0("WTG-", wt, " Power Curve - Rated Power: ", rated_power, " kW")) +
      scale_x_continuous(limits = c(min(csvPower$WindSpeed), max(csvPower$WindSpeed)),
                         breaks = seq(0, max(csvPower$WindSpeed), by = 1)) +
      # Expand y-axis to ensure rated power line is visible
      scale_y_continuous(limits = c(0, max(rated_power, max(csvPower$RealPower)) * 1.1),
                         breaks = seq(0, max(rated_power, max(csvPower$RealPower)) * 1.2, by = 200)) +
      theme_half_open() +
      background_grid()
  ...
  if (length(days) > 30) {
    # Roll up by month, keeping wind speed as a grouping variable
    csvPower <- csvPower %>%
        mutate(YearMonth = floor_date(TimeStamp10Min, "month"),
               WindSpeedBin = round(WindSpeed, 1)) %>%  # Round to nearest 0.1 m/s
        group_by(YearMonth, WindSpeedBin) %>%
        summarise(
            WindSpeed = round(mean(WindSpeed, na.rm = TRUE), digits = 2),
            RealPower = round(mean(RealPower, na.rm = TRUE), digits = 2),
            Rating = first(Rating),
            .groups = "drop"
        ) %>%
        select(-WindSpeedBin) %>%
        rename(TimeStamp10Min = YearMonth)
    
    message("Rolled up to monthly averages by wind speed bin")
  }
...
for (i in 1:NROW(wtgs)) {
  ...
  csvPower <- subset(csv.data, csv.data$WTG == wt & csv.data$RealPower > 0)
  if (nrow(csvPower) == 0) {
    next  # Skip to the next iteration if csvPower has 0 rows
  }
  rated_power <- unique(csvPower$Rating)
  powercurve <- csvPower %>%
      ggplot(aes(x = WindSpeed, y = RealPower)) +
